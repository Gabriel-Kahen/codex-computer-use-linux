...
pub(crate) async fn list_windows_with_policy(policy: WindowListPolicy) -> Result<Vec<WindowInfo>> {
    ...
    if let Some(backend) = preferred_backend {
        ...
        {
            let windows = if backend == BackendKind::GnomeIntrospect {
                let x11_result = if x11::can_exact_focus() {
                    Some(x11::list_windows_for_exact_focus().await)
                } else {
                    None
                };
                prefer_exact_x11_windows(windows, x11_result, &mut errors)
            } else {
                windows
            };
    ...
    for backend in BACKEND_ORDER {
            ...
            usable_backend_windows(*backend, list_windows_for(*backend).await, &mut errors)
        {
            if matches!(backend, BackendKind::GnomeIntrospect) {
                ...
                let windows = prefer_exact_x11_windows(windows, x11_result, &mut errors);
                window_cache().record_success(
                    generation,
                    *backend,
                    windows.clone(),
                    Instant::now(),
...
pub async fn focused_window_for_backend(backend: &str) -> Result<Option<WindowInfo>> {
    let windows = match backend {
        ...
        NIRI_BACKEND => niri::list_windows()?,
