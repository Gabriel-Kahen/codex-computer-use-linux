...
trait WindowRegistry: Sync + Sized {
    fn list_windows(
        &self,
    ...
    fn focused_window_override(&self) -> impl Future<Output = Option<WindowInfo>> + Send;

    fn focused_window_for_backend(
        &self,
        _backend: &str,
    ) -> impl Future<Output = Result<Option<WindowInfo>>> + Send {
...
impl WindowRegistry for BackendRegistry {
    ...
    async fn focused_window_override(&self) -> Option<WindowInfo> {
        registry::focused_window_override().await
    }

    async fn focused_window_for_backend(&self, backend: &str) -> Result<Option<WindowInfo>> {
        registry::focused_window_for_backend(backend).await
    }
}
...
async fn current_focused_window(
    ...
    if let Some(window) = window_registry.focused_window_override().await {
...
async fn wait_for_focused_window(
    ...
    wait_for_focused_window_with(requested_window, FOCUS_VERIFY_TIMEOUT, || {
        window_registry.focused_window_for_backend(&requested_window.backend)
    })
    .await
}
...
mod tests {
    ...
    fn window(window_id: u64, app_id: Option<&str>) -> WindowInfo {
        WindowInfo {
            ...
            hidden: false,
            client_type: None,
            // The fake registry needs an exact backend without live X11 session probes.
            backend: registry::HYPRLAND_BACKEND.to_string(),
            terminal: None,
        }
    }
