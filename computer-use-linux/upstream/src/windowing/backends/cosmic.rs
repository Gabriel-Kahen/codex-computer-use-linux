...
pub async fn list_windows() -> Result<Vec<WindowInfo>> {
    let json = tokio::task::spawn_blocking(cosmic_helper::list_windows_json).await??;
...
pub async fn focused_window() -> Result<Option<WindowInfo>> {
    let json = tokio::task::spawn_blocking(cosmic_helper::focused_window_json).await??;
...
pub async fn activate_window(window_id: u64) -> Result<()> {
    let activation =
        tokio::task::spawn_blocking(move || cosmic_helper::activate_window(window_id)).await??;
    if activation.ok {
        Ok(())
    } else {
