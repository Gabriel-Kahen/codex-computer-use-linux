...
/// `gnome-screenshot`.
const SCREENSHOT_BACKEND_ENVS: &[&str] = &[
    "COMPUTER_USE_LINUX_SCREENSHOT_BACKEND",
    "CODEX_COMPUTER_USE_SCREENSHOT_BACKEND",
];
...
fn forced_backend() -> Result<Option<ScreenshotBackend>> {
    let selected = SCREENSHOT_BACKEND_ENVS.iter().find_map(|name| {
        std::env::var(name)
            .ok()
    ...
    match selected {
        Some((name, value)) => ScreenshotBackend::parse(&value).map(Some).ok_or_else(|| {
            anyhow!(
                "{name}={value:?} is not a recognized backend \
...
async fn capture_with_gnome_extension() -> Result<RawScreenshotCapture> {
    ...
    .await
    .context("failed to create GNOME Shell extension proxy")?;
    let (ok, message): (bool, String) = match proxy.call("CaptureScreenshot", &(filename)).await {
        Ok(result) => result,
        Err(error) => {
            cleanup_gnome_requested_path(&path);
            return Err(error).context("GNOME Shell extension CaptureScreenshot call failed");
        }
    };
    if !ok {
        cleanup_gnome_requested_path(&path);
        bail!("GNOME Shell extension refused screenshot: {message}");
    }
...
mod tests {
    ...
    fn forced_backend_reads_env_override() {
        // Only this test touches the generic screenshot override, so no cross-test race.
        let screenshot_backend_env = SCREENSHOT_BACKEND_ENVS[0];
