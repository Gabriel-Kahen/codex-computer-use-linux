...
pub async fn list_accessible_apps(limit: usize) -> Result<Vec<AccessibleAppSummary>> {
    let (conn, roots) = connect_with_roots(limit).await?;
...
async fn snapshot_with_hydration(
    app_name_or_bundle_identifier: Option<&str>,
    target_pid: Option<u32>,
    max_nodes: usize,
    max_depth: u32,
...
async fn snapshot_tree_inner(
    ...
    let (conn, roots) = connect_with_roots(MAX_DISCOVERY_ROOTS).await?;
    ...
    let (_, mut remaining_filter_reads, mut remaining_traversal_reads) =
    ...
    while let Some((object_ref, depth, parent_index)) = traversal.pop() {
        ...
        let node = read_node(&proxy, &object_ref, index, parent_index, depth, hydration).await;
...
async fn connect_with_roots(
    limit: usize,
) -> Result<(AccessibilityConnection, Vec<ObjectRefOwned>)> {
    let conn = cached_read_connection().await?;
    match registry_children(&conn, limit, &mut limit.clone()).await {
        Ok(roots) => Ok((conn, roots)),
        Err(_) => {
            invalidate_connection(&conn);
            let conn = cached_read_connection().await?;
            let roots = registry_children(&conn, limit, &mut limit.clone()).await?;
            Ok((conn, roots))
        }
    }
}
