...
| `COMPUTER_USE_LINUX_FORCE_PORTAL_POINTER` / `…_KEYBOARD` | Prefer the RemoteDesktop portal on Wayland. Forced pointer mode maps logical targeted coordinates through selected ScreenCast monitor streams; raw screenshot pixels and cross-monitor drags fail closed. `FORCE_YDOTOOL_POINTER` wins if both are set. |
