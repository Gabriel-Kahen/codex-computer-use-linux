...
Pass the originating `observation_id` with `perform_action` and `set_value`. Missing, expired, evicted, and same-target stale IDs are rejected, and a direct `element_identifier` must exist in that snapshot. `perform_action` also resolves the requested action from that snapshot and invokes the unique live action matching its field-preserving name/description fingerprint, so live action reordering or cross-field collisions cannot retarget it. `click` and `scroll` retain their existing latest-snapshot behavior in this compatibility stage.
