...
| KDE Plasma / KWin | persistent KWin DBus window bridge | Keeps one event-driven Plasma 5/6 window snapshot bridge loaded for the MCP process and reloads it when the `org.kde.KWin` bus owner changes; focus actions still use short-lived scripts. |
...
- **DBus where desktops expose it** — [`zbus`](https://crates.io/crates/zbus) for portal calls (`org.freedesktop.portal.Screenshot`, `…RemoteDesktop`, `…ScreenCast`), GNOME Shell screenshots (`org.gnome.Shell.Screenshot`), the bundled GNOME extension's `dev.avifenesh.ComputerUseLinux.WindowControl` service, and a process-lifetime event-driven KWin window bridge. KWin focus actions remain short-lived scripts because KWin scripting does not expose a stable inbound command channel.
- **Portal session reuse** — pointer and keyboard portal actions share one lazily created RemoteDesktop session. Version-2 restore tokens are atomically rotated under `${XDG_STATE_HOME:-$HOME/.local/state}/computer-use-linux/` with directory/file modes `0700`/`0600`; a missing, stale, or revoked token falls back to the portal's normal consent flow.
