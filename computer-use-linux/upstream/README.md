...
- **Wayland actually works.** Coordinate-bearing pointer actions, including observation-bound element clicks, use the absolute uinput backend or `ydotool` / `ydotoold`. When those are unavailable, the `org.freedesktop.portal.RemoteDesktop` path shares its session with ScreenCast and maps logical AT-SPI/window coordinates through the selected monitor stream for clicks and targeted scrolls. Raw screenshot pixels, including drag coordinates, still fail closed on mixed-scale layouts because they cannot be inferred from a separate Screenshot portal image. Portal pointer and keyboard access share one process-lifetime session, and version-2 portals receive a private, single-use restore token so later processes can restore permission without another prompt when the desktop allows it. Screenshots use the GNOME Shell DBus screenshot method when present, `org.freedesktop.portal.Screenshot` otherwise, and fall back to spawning `gnome-screenshot` for background/systemd contexts where both DBus paths are denied.
...
- `screenshot` — capture the screen as a bounded PNG or JPEG image; COSMIC and Hyprland use compositor-native exact window capture when available, while other targeted paths may raise and crop the window
...
| --- | --- | --- |
| Read-only observation | `doctor`, `list_apps`, `list_windows`, `focused_window`, `get_app_state`, `list_window_claims` | `readOnlyHint=true`; may reveal app, window, accessibility, screenshot, and token-redacted claim ownership. `get_app_state` may trigger the desktop screenshot portal prompt. |
...
| Claim lifecycle mutators | `claim_window`, `renew_window_claim`, `release_window_claim` | `readOnlyHint=false`, `destructiveHint=false`, `openWorldHint=false`; changes private coordination state. Release is retry-idempotent; create and renew are not. |
...
`computer-use-linux` exposes `claim_window`, `renew_window_claim`, and `release_window_claim` for backend-neutral cross-process ownership on GNOME, Plasma, COSMIC, Niri, i3, and X11. Claim ownership comes from host-provided task metadata; successful create/renew calls return private credentials for window-scoped tools. Hyprland continues to use its companion broker during migration. Capture, focus, semantic action, value, pointer, keyboard, text, and action-batch tools expose optional `owner_thread_id` and `claim_token` fields; drag also accepts an additive window target. If a target window has a live claim, the generic server takes the same per-window file lock and rejects work without the matching owner and token. Focus changes, physical-seat input, and screenshots that raise their target take the shared global input lane before the window lock; pass `raise_window=false` for non-focusing exact capture. Untargeted capture and global input are rejected while any window claim is live. Combined action-and-observe calls keep the required guards across the mutation and observation. Unclaimed sessions keep their existing single-agent behavior.

For sustained exact-window work, exhaust `list_window_claims` pages by following `next_cursor`, create a claim, retain its token privately, renew before `expires_at_ms`, and release it in finally-style cleanup. List pages contain at most eight live claims and never expose tokens. Entries include both the public numeric `window_id` used by MCP tools and the backend-stable identity. If a listed claim is owned by the caller but its token was lost, stop and wait for expiry before reclaiming; if another task owns it, choose another window or wait. Lifecycle and list failures return bounded messages plus a bounded `next_action`. The `doctor.readiness.window_claims` block reports backend support, lease bounds, authoritative owner source, and the next workflow step.

When the version-1 `same-session-target-pointer` Hyprland extension is loaded, an eligible leading Wayland coordinate click in `run_action_batch` or `run_action_batch_and_observe` is sent through one identity-bound `cutargetbatch` compositor transaction without moving physical pointer, focus, or workspace. The same claim/window/global-seat guards remain held around the entire shared batch and observation. A missing native command before submission falls back to the established input path; identity/ABI refusal fails closed, and any failure after compositor submission is never replayed. Semantic clicks, keyboard/text actions, unsupported buttons/counts, and XWayland remain explicit existing routes. Set `CODEX_HYPRLAND_TARGET_POINTER_PLUGIN` to an absolute compiled plugin path only when the companion has not already loaded it.
...
| Niri | direct `NIRI_SOCKET` event stream and actions; `niri msg` fallback | Uses Niri's complete event-stream snapshot and incremental updates without polling. Exact inactive capture binds the compositor ScreenCast stream to the same stable window ID and requires GStreamer PipeWire, base, and good plugins; it never substitutes monitor or desktop pixels. The `niri` command is only required for compatibility fallback. |
| i3 | persistent native i3 IPC; `i3-msg` fallback; optional `xprop` PID hydration | Lists and focuses over one event-subscribed IPC connection, reconnecting when the socket is replaced. |
...
| `COMPUTER_USE_LINUX_FORCE_PORTAL_POINTER` / `…_KEYBOARD` | Prefer the RemoteDesktop portal on Wayland. Forced pointer mode maps logical targeted coordinates through selected ScreenCast monitor streams; raw screenshot pixels, including drag coordinates, fail closed. `FORCE_YDOTOOL_POINTER` wins if both are set. |
...
- **DBus where desktops expose it** — [`zbus`](https://crates.io/crates/zbus) for portal calls (`org.freedesktop.portal.Screenshot`, `…RemoteDesktop`, `…ScreenCast`), GNOME Shell screenshots (`org.gnome.Shell.Screenshot`), the bundled GNOME extension's `dev.avifenesh.ComputerUseLinux.WindowControl` service, a process-lifetime event-driven KWin window bridge, and KWin ScreenShot2 exact capture. KWin focus actions remain short-lived scripts because KWin scripting does not expose a stable inbound command channel.
- **Portal session reuse** — pointer and keyboard portal actions share one lazily created RemoteDesktop session. Coordinate pointer sessions also call ScreenCast `SelectSources` on that same session and use each returned stream's logical `position` and `size` for `NotifyPointerMotionAbsolute`. Version-2 RemoteDesktop restore tokens are atomically rotated under `${XDG_STATE_HOME:-$HOME/.local/state}/computer-use-linux/` with directory/file modes `0700`/`0600`; a missing, stale, or revoked token falls back to the portal's normal consent flow.
- **MCP transport** — [`rmcp`](https://crates.io/crates/rmcp) with the `transport-io` feature; stdio framing, no network.
- **Input fallback** — coordinate-bearing pointer actions prefer absolute uinput or `ydotoold`; the remote-desktop portal can use ScreenCast's logical stream mapping for targeted coordinates. Separate raw screenshot pixels remain unsupported when monitor scales differ. `install.sh` can configure `ydotoold`; the `setup` command only enables the GNOME AT-SPI bridge.
...
- **Native i3 transport** — i3 window listing, focus reads, and activation share one persistent IPC connection subscribed to window, workspace, output, and shutdown events. Events invalidate a bounded tree snapshot; socket path and inode checks reconnect after i3 restarts. `i3-msg` remains a compatibility fallback, while `_NET_WM_PID` hydration uses a per-window cache around optional `xprop` calls.
...
- **COSMIC helper** — `computer-use-linux-cosmic` talks to COSMIC toplevel and image-copy protocols and is resolved from `COMPUTER_USE_LINUX_COSMIC_HELPER`, next to the running binary, or from `PATH`. Normal window operations and captures reuse one helper process and Wayland connection; EOF, timeout, or invalid service responses trigger one automatic restart. Compatibility failures then use the one-shot command path, while repeated timeouts fail closed rather than starting another potentially wedged process. Activation is reported only when COSMIC confirms that the requested toplevel became focused. Exact capture creates a source from the live foreign-toplevel handle, supports only bounded 8-bit SHM buffers, rejects stale or minimized targets, and never falls back to a desktop crop.
- **Niri exact capture** — targeted Niri screenshots call the compositor's `org.gnome.Mutter.ScreenCast.RecordWindow` with the stable Niri IPC window ID, capture one PipeWire frame through `gst-launch-1.0`, and validate the window identity both before and after capture. Missing plugins, a missing ScreenCast service, a stale/closed target, or any hidden state reported by the backend is an error; monitor and desktop pixels are never substituted. Current Niri IPC does not expose a minimized-window state, so no minimized-window guarantee is inferred beyond the compositor accepting the exact window stream. Niri `block-out-from "screencast"` policy is honored and may intentionally produce black window pixels. `./install.sh` installs `pipewiresrc`, `videoconvert`, and `pngenc` on detected Niri sessions. Other install methods must provide the corresponding GStreamer PipeWire, base, and good plugin packages.
...
- **Niri's compositor ScreenCast service is a same-user session-bus interface.** Exact Niri window capture calls it directly with the stable IPC window ID and does not show the desktop portal chooser. Treat any process in the graphical login that can access this service as able to request the same window stream; Niri's `block-out-from "screencast"` rules still apply.
