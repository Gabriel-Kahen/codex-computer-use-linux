...
On Hyprland, `computer-use-linux` participates in the companion broker's cross-process window claims for capture and mutation. Capture, focus, semantic action, value, pointer, keyboard, text, and action-batch tools expose optional `owner_thread_id` and `claim_token` fields; drag also accepts an additive window target. If a target window has a live claim, the generic server takes the same per-window file lock and rejects work without the matching owner and token. Focus changes, physical-seat input, and screenshots that raise their target take the broker's shared global input lane before the window lock; pass `raise_window=false` for non-focusing exact capture. Untargeted capture and global input are rejected while any window claim is live. Combined action-and-observe calls keep the required guards across the mutation and observation. Unclaimed sessions keep their existing single-agent behavior.
