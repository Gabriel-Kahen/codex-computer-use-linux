...
On Hyprland, `computer-use-linux` also participates in the companion broker's cross-process window claims for standalone screenshot capture. `get_app_state` and `screenshot` expose optional `owner_thread_id` and `claim_token` fields. If a target window has a live claim, the generic server takes the same per-window file lock and rejects capture without the matching owner and token. A targeted `screenshot` that raises the window also takes the broker's global input lane before the window lock; pass `raise_window=false` for non-focusing exact capture. Untargeted desktop capture is rejected while any window claim is live. Unclaimed sessions keep their existing single-agent behavior. Mutation tools, including combined action-and-observe batches, do not yet consume this coordination context.
