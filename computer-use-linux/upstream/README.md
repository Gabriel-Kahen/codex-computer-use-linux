...
Pass the originating `observation_id` with element-targeted `click`, `perform_action`, and `set_value`. Missing, expired, evicted, target-mismatched, and same-target stale IDs are rejected. Element clicks are reverified against the exact window, snapshot node, and live AT-SPI bounds before pointer injection. `perform_action` also invokes the unique live action matching its observed name/description fingerprint. `scroll` retains its latest-snapshot behavior.
