...
class ParallelBackendTests(TestCase):
    ...
    def test_call_tool_maps_pointer_tools_to_actions_and_owner(self) -> None:
        observed: list[tuple[dict[str, object], str, str | None]] = []
        ...
        def target(
            ...
            observed.append((arguments, action, owner_thread_id))
            return {"action": action}

        with patch.object(server, "targeted_pointer", side_effect=target):
            results = [
                server.call_tool(
                    tool_name,
                    {"window": "0x1"},
        ...
        self.assertEqual(
            observed,
            [
                ({"window": "0x1"}, "click", "owner-a"),
                ({"window": "0x1"}, "scroll", "owner-a"),
                ({"window": "0x1"}, "drag", "owner-a"),
            ],
        )
