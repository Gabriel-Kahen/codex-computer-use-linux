...
Native Wayland windows have independent broker lanes, so different agents can issue captures and targeted actions concurrently. The same window is exclusive across broker processes, and the generic Computer Use server shares the lane for screenshot capture. Claim, renewal, release, expiry takeover, and capture use that lock, preventing a claim-expiry time-of-check/time-of-use race. Generic capture calls present the broker-issued owner and fencing token; untargeted desktop capture fails closed while any claim is live. Generic mutations remain a follow-up stage and still require the skill's ownership policy.
