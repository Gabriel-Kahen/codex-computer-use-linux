...
Native Wayland windows have independent broker lanes, so different agents can issue captures and targeted actions concurrently. The same window is exclusive across broker processes. Claim, renewal, release, expiry takeover, and mutation all pass through that same window lane, preventing a claim-expiry time-of-check/time-of-use race. Separate AT-SPI and global-input tools do not consume these locks and must follow the skill's ownership policy.
