...
Native Wayland windows have independent broker lanes, so different agents can issue captures and targeted actions concurrently. The same window is exclusive across broker processes, and the generic Computer Use server shares the lane for screenshot capture. A generic screenshot that raises its target takes the global input lane before the window lane, while `raise_window=false` exact capture remains window-local. Claim, renewal, release, expiry takeover, and capture use those locks, preventing claim-expiry and focus/input time-of-check/time-of-use races. Generic capture calls present the broker-issued owner and fencing token; untargeted desktop capture fails closed while any claim is live. Generic mutations remain a follow-up stage and still require the skill's ownership policy.
