...
The broker identifies each caller from the host-only `tools/call.params._meta.threadId`; tool arguments cannot override that identity. A window claim has a 60-second default lease (configurable from 5 to 300 seconds), an opaque `claim_token`, and an absolute `expires_at`. Renewing from the same task requires the current token and does not rotate it. Another task cannot capture or mutate the claimed window through this broker, even if it omits the token. Supplying the token adds explicit fencing, and a stale or wrong-window token is rejected.

Claim state uses the backend-neutral version-2 protocol, is atomically replaced, mode `0600`, and scoped to the real user's Wayland display, Hyprland instance, window address, and process start time. Cross-process file locks make same-window claim and mutation races deterministic. During a rolling upgrade the broker also takes the legacy address lock and refuses to replace version-1 state until every old claim is inactive. Different native Wayland windows use separate locks and can progress concurrently. Same-window operations remain serialized. XWayland, global-seat, and coordinate-fallback operations use one global lane because those paths share compositor or XTEST state. The generic Computer Use server takes that same global lane before its window lock for focus changes, physical-seat input, and screenshots that raise their target, and consumes the same claim state and window locks for capture, focus, AT-SPI, portal, pointer, keyboard, text, and combined action-and-observe operations: pass the claim result's `owner_thread_id` and `claim_token` to those tools. Pass `raise_window=false` to keep exact capture non-focusing.

The native extension handles each Wayland pointer action as one compositor-thread `cutarget` transaction. It also exposes the versioned `cutargetbatch v1` ABI used by the shared Rust `run_action_batch` engine for eligible pointer subsets. A transaction validates the broker/plugin identity and live input-safety state, prevalidates every pointer action, snapshots physical cursor/focus/workspace state, sends the complete event sequence directly to the selected surface, restores pointer focus, and snapshots physical state again before returning. It never moves Hyprland's physical pointer. XWayland actions snapshot and restore XWayland's separate internal pointer and are not accepted by the native batch ABI.
...
3. Lets claimed broker operations renew automatically, and calls `claim_session_window` with the current token before `expires_at` during longer external semantic work. A renewal from the same task keeps the token stable.
...
The companion broker builds and loads the native extension on demand. Builds are cached outside the installed plugin under `${XDG_CACHE_HOME:-$HOME/.cache}/same-session-computer-use/` and keyed by the plugin source and running Hyprland version. The generic Rust backend reuses a compatible loaded extension; it can load an explicit prebuilt regular non-symlink `.so` from `CODEX_HYPRLAND_TARGET_POINTER_PLUGIN` when the command is absent. Both paths validate the versioned protocol, immutable plugin identity, and build/runtime ABI. Live lock, drag, pointer-grab, pointer-constraint, and physical-button safety state is never cached. An explicitly missing single-action command triggers one safe reload attempt in the companion, while the Rust batch path never replays after `cutargetbatch` submission.
