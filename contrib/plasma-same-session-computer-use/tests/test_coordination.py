...
class CoordinationTests(TestCase):
    def setUp(self) -> None:
        ...
        self.session_claims_dir = coordination._session_claims_dir(SESSION)
    ...
    def test_release_bounds_the_persisted_window_summary(self) -> None:
        claim = coordination.claim_window(window("{shared}"), "owner-a", 60)
        key = claim["claim_token"].split(".", 1)[0]
        path = self.session_claims_dir / f"{key}.json"
        record = coordination.read_private_json(path)
        assert record is not None
        record["window"]["title"] = "x" * 100_000
        record["window"]["private"] = "y" * 100_000
        coordination.write_private_json(path, record)

        result = coordination.release_window_claim(claim["claim_token"], "owner-a")

        self.assertEqual(set(result["window"]), {"id", "capture_id", "title", "class"})
        self.assertLessEqual(len(result["window"]["title"]), coordination.MAX_WINDOW_TITLE_CHARS)
    ...
    def test_focus_cleanup_does_not_remove_an_implicit_claim_converted_to_explicit(self) -> None:
        ...
        self.assertEqual(released, coordination.ClaimCleanupResult.BLOCKED)
        self.assertEqual(coordination.list_claims("owner-a", 0, 20)["total"], 1)

    def test_missing_bound_claim_cleanup_is_idempotent(self) -> None:
        claim = coordination.claim_window(window("{shared}"), "owner-a", 60)
        key = claim["claim_token"].split(".", 1)[0]
        (self.session_claims_dir / f"{key}.json").unlink()

        released = coordination.discard_bound_claim(claim["claim_token"], "{shared}", "owner-a")

        self.assertEqual(released, coordination.ClaimCleanupResult.ALREADY_ABSENT)
    ...
    def test_claim_state_from_another_kwin_session_is_preserved(self) -> None:
        claim = coordination.claim_window(window("{shared}"), "owner-a", 60)
        ...
        self.assertNotEqual(replacement["claim_token"], claim["claim_token"])
        self.assertEqual(len(list(self.claims_dir.rglob("*.json"))), 2)
