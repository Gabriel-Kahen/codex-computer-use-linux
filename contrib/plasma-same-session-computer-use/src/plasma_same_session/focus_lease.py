...
def _diagnostic(prefix: str, value: Any) -> str:
    return coordination._truncate_json_text(
        f"{prefix}{value}",
...
def _recorded_session_is_current(
    ...
    try:
        ...
        errors.append(_diagnostic(f"session identity {checkpoint}: ", exc))
        return False
...
def _finish_restore(
    ...
    if recovery_complete and claim.get("implicit") is True:
        try:
            ...
            claim_release_error = _diagnostic("", exc)
            errors.append(_diagnostic("implicit window claim cleanup: ", exc))
            recovery_complete = False
...
def restore(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if recorded_session:
        try:
            ...
            errors.append(_diagnostic("session identity: ", exc))
        ...
        try:
            ...
            if not verified["desktop"]:
                errors.append(_diagnostic(
                    ...
                    f"expected {original['desktop']}, observed {observed_desktop}",
                ))
        except Exception as exc:
            errors.append(_diagnostic("desktop verification: ", exc))
        ...
        try:
            ...
            if not verified["pointer"]:
                errors.append(_diagnostic(
                    ...
                    f"expected {original_pointer}, observed {observed_pointer}",
                ))
        except Exception as exc:
            errors.append(_diagnostic("pointer verification: ", exc))
    if not errors:
...
def validate(state: dict[str, Any], owner_id: str | None = None) -> dict[str, Any]:
    ...
    if claim:
        try:
            ...
            claim_active = False
            errors.append(_diagnostic("window claim: ", exc))
    try:
        live_ids = {window["id"] for window in kwin.list_windows()}
    except Exception as exc:
        errors.append(_diagnostic("window enumeration: ", exc))
