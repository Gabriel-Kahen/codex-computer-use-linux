...
The PNG returned to Codex is capped at 5 MiB so its base64-encoded MCP response remains below Codex's stdio transport limit. `get_plasma_window_capture` is read-only and returns its PNG inline. The separately destructive `save_plasma_window_capture` tool atomically replaces its absolute `save_path` only after a valid capture succeeds; an oversized capture leaves the destination untouched. The original `capture_plasma_window` tool remains as a deprecated compatibility route with its optional `save_path` behavior and destructive annotation.
