...
TOOLS = [
    ...
    tool("release_session_window", "Release this Codex agent's live window claim.", CLAIM_TOKEN, ["claim_token"], idempotent=True),
    ...
    tool("send_window_shortcut", "Best-effort no-focus XSendEvent shortcut delivery. Many modern clients reject synthetic events; use an acknowledged input lease when reliable delivery is required.", {**WINDOW, **CLAIM_TOKEN, "key": {"type": "string"}, "modifiers": {"type": "string", "default": ""}}, ["window", "key"]),
    tool("begin_input_lease", "Begin an explicit journaled focus/pointer lease for reliable XTEST input, snapshotting the active desktop, focus, pointer, and target minimized state for restoration.", {**WINDOW, **CLAIM_TOKEN, "acknowledge_interference": {"type": "boolean"}}, ["window", "acknowledge_interference"]),
    tool("lease_key", "Send a reliable key or shortcut while the acknowledged target input lease is active.", {**TOKEN, **CLAIM_TOKEN, "key": {"type": "string"}, "modifiers": {"type": "string", "default": ""}}, ["lease_token", "key"]),
