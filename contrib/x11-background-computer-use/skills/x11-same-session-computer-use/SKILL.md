...
4. Capture mapped, non-minimized windows inline with the read-only `get_session_window_capture`. Use `save_session_window_capture` only when the user needs a PNG written to an absolute path, and do not use the deprecated `capture_session_window` compatibility tool in new workflows. XComposite capture does not focus, raise, move, or uncover the window.
5. Prefer semantic operations from the separate `computer-use-linux@codex-computer-use-linux` plugin. Correlate AT-SPI applications using the returned PID, title, and WM_CLASS; never hardcode bus names or object paths.
