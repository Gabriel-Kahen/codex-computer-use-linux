...
4. Prefer semantic AT-SPI actions from `computer-use@openai-bundled`: actions first, then editable text/value operations. That companion runs outside this broker, so claims coordinate AT-SPI agents by policy and do not mechanically prevent a non-cooperating external action.
5. Exact capture of an already focused window is safe. An inactive window requires the one global focus/input lane.
