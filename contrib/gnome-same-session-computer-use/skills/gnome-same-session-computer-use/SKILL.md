...
5. Exact inline capture of a mapped focused or inactive window with the read-only `get_session_window_capture` is safe. A minimized window must fail closed on that path. When fresh minimized pixels are necessary, obtain explicit interference acknowledgment and use `capture_minimized_session_window`; it briefly activates the target under a recovery lease and may show its workspace, but returns no pixels unless client damage is painted and minimized state, workspace, focus, and pointer are restored. Use `save_session_window_capture` only when the user needs a mapped-window PNG written to an absolute path, and do not use the deprecated `capture_session_window` compatibility tool in new workflows.
6. Before `capture_minimized_session_window` or `begin_focus_lease`, obtain explicit interference acknowledgment from the user in the current task. A literal tool argument is not evidence of consent. Expect another agent's active minimized-capture or focus-lease transaction to make this lane temporarily unavailable; continue independent semantic work or retry after that agent restores it.
...
Pointer transactions briefly move Mutter's global virtual pointer to the target and restore the prior position immediately. The workspace and keyboard focus remain leased until cleanup. Mapped capture introduces no focus change. Fresh minimized capture deliberately uses a short focus/workspace activation transaction to solicit a new client frame, then verifies restoration before returning.
