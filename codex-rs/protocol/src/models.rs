...
impl CallToolResult {
    ...
    pub fn as_function_call_output_payload(&self) -> FunctionCallOutputPayload {
        ...
        // `structuredContent` cannot represent model-visible media or encrypted content. Keep
        // those MCP content blocks rather than silently discarding them in favor of structured
        // JSON. When media is present, retain the structured payload as model-visible text exactly
        // once so metadata such as dimensions and coordinate transforms remains usable.
...
- ["a", "b", "c"]"#
    ...
    #[test]
    fn preserves_mcp_audio_with_structured_content() {
        let call_tool_result = CallToolResult {
            content: vec![
                serde_json::json!({"type":"text","text":"caption"}),
                serde_json::json!({"type":"audio","data":"BASE64","mimeType":"audio/wav"}),
            ],
            structured_content: Some(serde_json::json!({"duration_ms": 1200})),
            is_error: Some(false),
            meta: None,
        };
